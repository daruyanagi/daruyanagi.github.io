...
- 自動更新が効くのは、メインのタイムラインだけです
- スクロールが一番上にない（Home キーを押すか、タイムラインのヘッダーをダブルクリックします
- 投稿を入力中である。投稿画面を表示状態にしており、そこで記入中の場合は、投稿を妨げないように一時停止します
- もしかして：Edge がアクティブでない場合、タブが休止するかもしれません。タブをアクティブにします
